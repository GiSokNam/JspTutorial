...
<head>
<meta charset="UTF-8">
<title>memberTutorial/loginPro.jsp</title>
</head>
<body>
<%
// request id, pass 파라미터 값 => 변수 저장
...
// else										   false => 아이디 비밀번호 틀림

if(rs.next()){
	// 데이터가 있으면 true => 아이디 비밀번호 일치
	out.print("아이디 비밀번호 일치");
	// 페이지 상관없이 id값을 유지 => session값 저장 "id"
	session.setAttribute("id", id);
	// 메인페이지 이동
	response.sendRedirect("main.jsp");
} else {
	// 데이터가 없으면 false => 아이디 비밀번호 틀림
